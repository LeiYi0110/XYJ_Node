...
router.get('/area_id/:area_id/startIndex/:startIndex/length/:length', function(req, res, next) {
    ...
    collection.count({"area_id":area_id},function(err, item) {
        ...
        if (startIndex < item)
        {
            collection.find({"area_id":area_id, "cover":{"$ne":""}},{"restaurant_desc": 0}).skip(startIndex).limit(length).toArray(function(err, items) {
                ...
                area_collection.findOne({"area_id":area_id},function(err, item) {
                    ...
                    collection.find({"gps_sphere_3":{ "$nearSphere": { "$geometry": { "type": "Point", "coordinates": [ longitude, latitude ] }} },"cover":{"$ne":""},"area_id":{"$ne":area_id}},{"restaurant_desc": 0}).skip(0).limit(startIndex + length - count).toArray(function(err, items) {
                        //res.json({"status":1, "data":items});  
                        if (item == null){
                            console.log(4);
                            res.json({"status":1,"data":resultData});
        ...
        else
        {
            ...
            area_collection.findOne({"area_id":area_id},function(err, item) {
                ...
                collection.find({"gps_sphere_3":{ "$nearSphere": { "$geometry": { "type": "Point", "coordinates": [ longitude, latitude ] }} },"cover":{"$ne":""},"area_id":{"$ne":area_id}},{"restaurant_desc": 0}).skip(startIndex - count).limit(length).toArray(function(err, items) {
                        //res.json({"status":1, "data":items});  
                    if (item == null){
                        res.json({"status":0,"data":[]});
                        return; 
...
router.get('/without/area_id/:area_id/startIndex/:startIndex/length/:length', function(req, res, next) {
    ...
    area_collection.findOne({"area_id":area_id},function(err, item) {
        ...
        collection.find({"gps":{"$near":[latitude,longitude]},"cover":{"$ne":""},"area_id":{"$ne":area_id}},{"restaurant_desc": 0}).skip(startIndex).limit(length).toArray(function(err, items) {
            console.log(3);
            res.json({"status":1, "data":items});
            console.log(4);
        });
